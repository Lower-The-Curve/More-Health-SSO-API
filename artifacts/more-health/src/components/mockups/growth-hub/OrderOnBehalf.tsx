...
export function OrderOnBehalf() {
  ...
  // members so the user can pick someone without remembering their name. This
  // query stays cached while the user types in the search box. When the search
  // box is empty both queries share the same key and React Query dedupes them.
  ...
  const selectMember = (m: DownlineMember) => {
    // Clear any active search so the selected member is present in the list
    // below and its order actions expand into view.
    setInput("");
    setQ("");
    setSelected(m);
  };
  ...
  // pre-built URLs reflect the latest settings) and clear it if it falls out
  // of the result set.
  useEffect(() => {
    if (!selected) return;
  ...
  return (
    <AppLayout activeId="order-behalf">
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">
            {t("Order on Behalf Of", "代客下单")}
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            {t(
              "Search your team, pick a customer, then place an order or enroll on their behalf. You pay from your wallet on the checkout page.",
              "搜索您的团队，选择一位客户，然后代其下单或注册。您将在结账页面使用钱包余额付款。",
            )}
          </p>
        </div>
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground max-w-sm">
                {q
                  ...
                  : t(
                      "Customers you enroll will appear here so you can order on their behalf.",
                      "您注册的客户将显示在这里，方便您代其下单。",
                    )}
              </p>
            </div>
          </Card>
                    ...
                    <div className="border-t border-border bg-secondary/30 p-4 space-y-3">
                      <div className="flex flex-col sm:flex-row gap-2">
                        <Button
                          ...
                          disabled={!m.shopOnBehalfUrl}
                          onClick={() => openUrl(m.shopOnBehalfUrl)}
                        >
                      ...
                      {!m.shopOnBehalfUrl || !m.enrollOnBehalfUrl ? (
                        <p className="text-xs text-muted-foreground">
                          {!m.email || !m.kwikApexId
                            ? t(
                                "This customer is missing an email or member ID, so an order can't be attributed to them yet.",
                                "该客户缺少邮箱或会员编号，暂时无法为其归属订单。",
                              )
                            : t(
                                "Some links aren't available yet — an admin needs to set the shop and enrollment URLs in Settings.",
                                "部分链接尚不可用——管理员需要在设置中配置商城和注册链接。",
                              )}
                        </p>
                      ) : null}
                    </div>

...
export function MobileSettings() {
  const { fullName, initials, imageUrl, rank, influencerId, user } = useCurrentUser();
        ...
        <div className="px-5 mt-2">
          <div className="bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border border-border/50 rounded-2xl p-4 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-base font-display font-semibold">{fullName}</p>
              <p className="text-[11px] text-muted-foreground">{hideInfluencerStatus ? (influencerId ? `ID ${influencerId}` : "") : `${rank}${influencerId ? ` · ID ${influencerId}` : ""}`}</p>
              {email ? <p className="text-[10px] uppercase tracking-wider text-primary font-semibold mt-1">{email}</p> : null}
            </div>
